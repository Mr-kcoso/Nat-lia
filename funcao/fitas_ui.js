...
// Confirm back-to-menu helper
FitasUI.confirmBackToMenu = function () {
    const overlay = document.createElement('div');
    overlay.className = 'fita-confirm-overlay';
    overlay.innerHTML = `
        <div class="confirm-box">
            <div class="confirm-title">B‑M0 › Confirmação</div>
            <div class="confirm-body">Deseja voltar ao menu? Progresso não salvo nesta fita será perdido.</div>
            <div class="confirm-buttons">
                <button class="confirm-btn cancel">CANCELAR</button>
                <button class="confirm-btn confirm">B‑M0 › RETORNAR</button>
            </div>
        </div>`;
    document.body.appendChild(overlay);
    // focus management
    ...
    function escHandler(ev) { if (ev.key === 'Escape') remove(); }
    document.addEventListener('keydown', escHandler);

    cancelBtn.addEventListener('click', () => { remove(); });
    confirmBtn.addEventListener('click', () => { window.location.href = '../menu.html'; });
};
